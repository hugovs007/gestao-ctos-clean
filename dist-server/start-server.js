...
async function startServer() {
    ...
    app.get("/api/cities/:cityId/ctos", async (req, res) => {
        try {
            ...
            const ctosWithStats = await Promise.all(ctos.map(async (cto) => {
    ...
    app.post("/api/ctos", async (req, res) => {
        ...
        try {
            const result = await pool.query("INSERT INTO ctos (name, city_id, total_ports, address) VALUES ($1, $2, $3, $4) RETURNING id", [name, city_id, total_ports || 16, address]);
    ...
    app.post("/api/clients", async (req, res) => {
        ...
        try {
            const result = await pool.query("INSERT INTO clients (name, address, pppoe, city_id, cto_id, port_number, status) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id", [name, address, pppoe, city_id, cto_id, port_number, status || 'active']);
