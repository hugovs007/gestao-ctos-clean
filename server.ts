  ...
  try {
    ...
    app.get("/api/cities/:cityId/ctos", async (req, res) => {
      try {
        ...
        const ctosWithStats = await Promise.all(
          ctosResult.rows.map(async (cto: any) => {
    ...
    app.post("/api/ctos", async (req, res) => {
      ...
      try {
        const result = await pool.query(
          "INSERT INTO ctos (name, city_id, total_ports, address) VALUES ($1, $2, $3, $4) RETURNING *",
    ...
    app.get("/api/ctos/:id", async (req, res) => {
      try {
        const ctoResult = await pool.query(
          "SELECT * FROM ctos WHERE id = $1",
          [req.params.id]
        );
        ...
        const clientsResult = await pool.query(
          "SELECT * FROM clients WHERE cto_id = $1",
          [req.params.id]
        );
    ...
    app.post("/api/clients", async (req, res) => {
      ...
      try {
        const result = await pool.query(
          `INSERT INTO clients (name, address, pppoe, city_id, cto_id, port_number, status) 
